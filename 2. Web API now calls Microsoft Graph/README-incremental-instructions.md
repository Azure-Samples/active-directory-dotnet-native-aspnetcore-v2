...
If you want to use this automation, read the instructions in [App Creation Scripts](./AppCreationScripts/AppCreationScripts.md), but once you've run the script be sure to follow the manual steps. Indeed Azure AD PowerShell does not yet provide full control on applications consuming v2.0 tokens. This registration is already possible from the Azure portal:

- edit the **manifest** of the TodoListClient application to ensure that the `signInAudience` property is set to `AzureADandPersonalMicrosoftAccount`
- edit the **manifest** of the TodoListService application to ensure that:
  - `accessTokenAcceptedVersion` is set to  **2**
  - `signInAudience` is set to `AzureADandPersonalMicrosoftAccount`
- in the **Authentication** page for the TodoListService application, check the `urn:ietf:wg:oauth:2.0:oob` reply URI so that the client can propose incremental consent to the user for the Web API when needed.
...
- In the `ConfigureServices` method, replace:
  ...
  Note the replacement of BearerAuthenticationScheme by **Jwt**BearerAuthenticationScheme

  The services that are added are:

  - a token acquisition service leveraging MSAL.NET
  - a token cache based on the Web API session

  The implementations of these classes are in the Extensions subfolder, and they are designed to be reusable in your applications (Web apps and Web apis)

- Still on the `ConfigureServices` method, after
  ...
  insert the following code. It ensures that a JWT token received by the Web API is added to MSAL token cache. From there, the controllers will use  MSAL.NET on-behalf-of flow  later to acquire a token for the downstream APIs.
...
The code for this method is available in [TodoListService\Extensions\TokenAcquisition.cs L347-389](https://github.com/Azure-Samples/active-directory-dotnet-native-aspnetcore-v2/blob/9be6d8b3705ada352caa9a47be0a5add8389b6f6/2.%20Web%20API%20now%20calls%20Microsoft%20Graph/TodoListService/Extensions/TokenAcquisition.cs#L347-L380)
