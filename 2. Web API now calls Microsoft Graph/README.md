...
1. Find the app key `ClientId` and replace the existing value with the application ID (clientId) of the `TodoListService(ms-identity-dotnet-native-aspnetcore-v2)` application copied from the Microsoft Entra admin center.
1. Find the app key `ClientSecret` and replace the existing value with the key you saved during the creation of the `TodoListService(ms-identity-dotnet-native-aspnetcore-v2)` app, in the Microsoft Entra admin center.
...
1. Find the app key `ida:ClientId` and replace the existing value with the application ID (clientId) of the `TodoListClient(ms-identity-dotnet-native-aspnetcore-v2)` application copied from the Microsoft Entra admin center.
